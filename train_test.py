...
def net_train(model, hex, cross_entropy, optimizer, scheduler, balance, epochs, device, train_data_loader, test_data_loader, train_data_size, test_data_size, proportion):
    # Create Tensorboard
    writer = SummaryWriter('runs_' + str(balance) + '_' + str(proportion))
    ...
    for epoch in range(epochs):
        ...
        print('Balance:{}'.format(balance) + '  Epoch: {}/{}'.format(epoch, epochs) + '  Learning rate: {}'.format(optimizer.param_groups[0]['lr']))
        ...
        for i, (inputs, labels) in enumerate(train_data_loader):
            ...
            optimizer.zero_grad()
            fc_leaf, fc_total = model(inputs)
            loss = total_loss(fc_leaf, fc_total, hex, cross_entropy, balance, labels, device)
            loss.backward()
            optimizer.step()

            train_loss += loss.item() * inputs.size(0)
            train_corrects = evaluation(train_corrects, hex, fc_leaf, fc_total, labels, balance, device)
        ...
        with torch.no_grad():
            ...
            for j, (inputs, labels) in enumerate(test_data_loader):
                ...
                test_corrects = evaluation(test_corrects, hex, fc_leaf, fc_total, labels, balance, device)
        ...
        epoch_end = time.time()
        print('Balance: {:.2f}, Epoch: {:03d}, Training Loss: {:.4f}, Training Accuracy: {:.4f}%, Time: {:.4f}s'.format(balance,
                                                                                                                        epoch,
                                                                                                                        avg_train_loss,
                                                                                                                        avg_train_acc * 100,
                                                                                                                        epoch_end - epoch_start))
        print('Balance: {:.2f}, Epoch: {:03d}, Test Loss: {:.4f}, Test Accuracy: {:.4f}%'.format(balance,
                                                                                                 epoch,
                                                                                                 avg_test_loss,
                                                                                                 avg_test_acc * 100))

    writer.close()
    print('Best Accuracy on the test set: {:.4f}% at epoch: {:03d} of Balance: {:.2f}'.format(best_acc * 100,
                                                                                              best_epoch,
                                                                                              balance))
...
def net_test(model, device, test_data_loader, num_classes, class_names, hex, balance):
    # Overall Accuracy
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        ...
        for j, (images, labels) in enumerate(test_data_loader):
            ...
            fc_leaf, fc_total = model(images)

            # print(imagename, end="")
            # print('\n')
            # print(leaf_labels.cpu().numpy(), end="")
            # print('\n')
            # print(torch.exp(fc_leaf).cpu().numpy(), end="")
            # print('\n')
            # print(fc_total.cpu().numpy(), end="")
            # print('\n')
            ...
            total_p = fc_leaf.data + balance * pMargin
            _, predicted = torch.max(total_p.data, 1)
            # _, predicted = torch.max(fc_leaf.data, 1)
...
def total_loss(fc_leaf, fc_total, hex, cross_entropy, balance, labels, device):
    leaf_labels = torch.nonzero(labels > 2)   # CUB2011 class: 122-321; HRSC2016/FGSC class: 3-23
    hex_loss = hex(fc_total, labels, device)
    if leaf_labels.shape[0] > 0:
        select_leaf_labels = torch.index_select(labels, 0, leaf_labels.squeeze()) - 3
        select_fc_leaf = torch.index_select(fc_leaf, 0, leaf_labels.squeeze())
        cross_entropy_loss = cross_entropy(select_fc_leaf.to(torch.float64), select_leaf_labels)
        loss = cross_entropy_loss + balance * hex_loss
    else:
        loss = hex_loss
    return loss


def evaluation(corrects, hex, fc_leaf, fc_total, labels, balance, device):
    with torch.no_grad():
        leaf_labels = torch.nonzero(labels > 2)  # CUB2011 class: 122-321; HRSC2016/FGSC class: 3-23
        if leaf_labels.shape[0] > 0:
            select_leaf_labels = torch.index_select(labels, 0, leaf_labels.squeeze()) - 3
            ...
            total_p = select_fc_leaf.data + balance * pMargin
            probs, predictions = torch.max(total_p.data, 1)
...
def test_AP(model, device, test_set, test_data_loader, balance, hex, mode):
    total = 0.0
    correct = 0.0
    with torch.no_grad():
        model.eval()
        for j, (images, labels) in enumerate(test_data_loader):
            ...
            if mode == 0:
                ...
                # Sig combined
                batch_pMargin = fc_total
                batch_pMargin[:, 3:] = fc_leaf + balance * fc_total[:, 3:]
            elif mode == 2:
                # Mar fc_total
                batch_pMargin = hex.inference(fc_total, device)
                batch_pMargin = batch_pMargin.data.T
                ...
                # Mar combined
                fc_mar = hex.inference(fc_total, device)
                fc_mar = fc_mar.data.T
                batch_pMargin = fc_mar
                batch_pMargin[:, 3:] = fc_leaf.to(torch.float64) + balance * fc_mar[:, 3:]
            else:
